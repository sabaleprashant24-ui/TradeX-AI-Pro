...
logger = logging.getLogger("TradeX_Strategies")
...
class MultiFactorStrategy:
    ...
    def evaluate(df: pd.DataFrame, min_score_threshold: Optional[float] = None) -> Dict[str, Any]:
        ...
        # Fetch weights from config or use defaults
        weights = STRATEGY_CONFIG.get("weights", {})
        threshold = min_score_threshold or STRATEGY_CONFIG.get("min_score_threshold", 65.0)

        # Ensure indicators are present (Reuses existing columns)
        if "supertrend" not in df.columns or "rsi" not in df.columns:
...
class StandardStrategies:
    ...
    def trend_following(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        df = INDICATORS.add_all_indicators(df, clean_nan=True) if "supertrend" not in df.columns else df
        latest = df.iloc[-1]
